  ...
  Widget build(BuildContext context) {
    ...
    order.rawAdditionalDetails.forEach((key, value) {
      if (!predefinedKeys.contains(key) && key.isNotEmpty) {
        ...
        if (lowerKey.contains('type') || lowerKey.contains('mode') || lowerKey.contains('status')) {
            ...
            child: pw.Column(
              ...
              children: [
                  ...
                  children: [
                    pw.Column(
                      ...
                      children: [
                        ...
                        if (order.email != null && order.email!.isNotEmpty) ...[
                          pw.SizedBox(height: 2),
                          pw.Text('Email: ${order.email}', style: const pw.TextStyle(fontSize: 11)),
                        ],
                      ],
                    ),
                    pw.Column(
                      ...
                      children: [
                        ...
                        pw.Text(
                          ...
                          style: pw.TextStyle(
                            fontSize: 12,
                            fontWeight: pw.FontWeight.bold,
                            color: order.status == OrderStatus.completed || order.status == OrderStatus.delivered
                                ? PdfColors.green700
                ...
                pw.Container(
                  ...
                  child: pw.Column(
                    children: [
                      _buildPdfRow('Booking Type', order.bookingType ?? 'General'),
                      if (order.bookingType == 'Appointment Booking') ...[
                        _buildPdfRow('Date / Time', '${order.appointmentDate ?? "N/A"} @ ${order.appointmentTime ?? "N/A"}'),
                        _buildPdfRow('Platform', order.platform ?? 'N/A'),
                        _buildPdfRow('Duration', order.duration ?? 'N/A'),
                      ] else if (order.bookingType == 'Parcel Delivery') ...[
                        _buildPdfRow('Pickup Address', order.pickupAddress ?? 'N/A'),
                        _buildPdfRow('Delivery Address', order.deliveryAddress ?? 'N/A'),
                        _buildPdfRow('Product Type', order.productType ?? 'N/A'),
                        _buildPdfRow('Height / Weight', '${order.productHeight ?? "N/A"} / ${order.productWeight ?? "N/A"} kg'),
                        _buildPdfRow('Receiver Phone', order.receiverPhone ?? 'N/A'),
                      ] else if (order.bookingType == 'Order Booking') ...[
                        _buildPdfRow('Company Name', order.companyName ?? 'N/A'),
                        _buildPdfRow('Delivery Date', order.deliveryDate ?? 'N/A'),
                        _buildPdfRow('Delivery Address', order.deliveryAddress ?? 'N/A'),
                        _buildPdfRow('Product Type', order.productType ?? 'N/A'),
                      ],
                    ],
                ...
                if (order.rawAdditionalDetails.entries.any((e) => !predefinedKeys.contains(e.key) && e.key.isNotEmpty && e.value != null && e.value.toString().isNotEmpty)) ...[
                  pw.Text(
                    'ADDITIONAL DETAILS',
                    style: pw.TextStyle(
                      fontSize: 12,
                  ...
                  pw.Container(
                    ...
                    child: pw.Column(
                      children: order.rawAdditionalDetails.entries
                          .where((e) => !predefinedKeys.contains(e.key) && e.key.isNotEmpty && e.value != null && e.value.toString().isNotEmpty)
                          .map((e) {
                  ...
                  children: [
                    pw.TableRow(
                      ...
                      children: [
                        ...
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(8),
                          child: pw.Text('Txn ID / Details', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
                        ),
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(8),
                    ...
                    pw.TableRow(
                      children: [
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(8),
                          child: pw.Text('Booking Service Fee (${order.bookingType ?? "General"})'),
                        ),
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(8),
                        ...
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(8),
                          child: pw.Text('\$${order.price}', textAlign: pw.TextAlign.right),
                        ),
                      ],
                    ),
                  ],
                        ...
                        children: [
                          pw.Row(
                            ...
                            children: [
                              ...
                              pw.Text('\$${order.price}', style: const pw.TextStyle(fontSize: 12)),
                            ],
                          ),
                          pw.SizedBox(height: 4),
                          pw.Row(
                            mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                            children: [
                              pw.Text('Tax / Vat (0%):', style: const pw.TextStyle(fontSize: 12)),
                              pw.Text('\$0.00', style: const pw.TextStyle(fontSize: 12)),
                            ],
                          ),
